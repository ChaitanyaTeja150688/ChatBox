...
const preLoad = function () {
  console.log('[Code-Sages] Install Event processing');
  return caches.open('code-sages-offline').then(function (cache) {
    console.log('[Code-Sages] Cached index and offline page during Install');
...
self.addEventListener('fetch', function (event) {
  ...
  new Promise((res, rej) => {
    return s = CACHE_DOMAINS.filter((d, i) => {
...
self.addEventListener('push', (e) => {
  ...
  e.waitUntil(
    self.registration.showNotification('Code Sages', options)
  );
});
...
const addToCache = function (request) {
  return caches.open('code-sages-offline').then(function (cache) {
    return fetch(request).then(function (response) {
      console.log('[code-sages] add page to offline => ' + response.url);
